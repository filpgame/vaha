...
include './includes/info_logado.php';
    ...
    //$result = "UPDATE `estatiticas` SET `acertos`= (SELECT SUM(acertos) + $acertosAtuais), erros = (SELECT SUM(erros) + $errosAtuais), pontuacao_geral = (SELECT SUM(pontuacao_geral)+ $pontuacaoAtual)  WHERE ID_usuario = $ID_usuario";
